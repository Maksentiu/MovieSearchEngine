...
public class MovieService {
    ...
    public MovieDto movieToMovieDto(Movie movie) {
        ...
        // Преобразование списка актеров в DTO
        movieDto.setActors(movie.getActors().stream()
                .map(actorService::actorToActorDto) // Предположим, что у вас есть метод actorToActorDto в ActorService
                .collect(Collectors.toSet()));

        // Преобразование списка комментариев в DTO
        movieDto.setComments(movie.getComments().stream()
                .map(commentService::commentToCommentDto) // Предположим, что у вас есть метод commentToCommentDto в CommentService
                .collect(Collectors.toSet()));
